...
def parse_config():
    ...
    # choose dataset
    parser.add_argument('--choose_dataset', choices=['none', 'kitti', 'nuscenes'], default='none')
...
def eval_one_epoch_raw(model, test_loader, args, eval_output_dir, logger, epoch_id, dist_test=False, data_mode='kitti'):
    # load checkpoint
    ...
    raw_data_utils.eval_one_epoch_raw(
        ...
        result_dir=eval_output_dir, data_mode=data_mode
    )
...
def main():
    ...
    with torch.no_grad():
        if args.choose_dataset != 'none':
            if args.eval_all:
                repeat_eval_ckpt(model, test_loader, args, eval_output_dir, logger, ckpt_dir, dist_test=dist_test)
            else:
                eval_one_epoch_raw(model, test_loader, args, eval_output_dir, logger, epoch_id, dist_test=dist_test, data_mode=args.choose_dataset)
                # if args.raw_output:
            ...
            logger.info('Choose a dataset from kitti & nuscenes')
